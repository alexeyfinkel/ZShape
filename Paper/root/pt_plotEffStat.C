...
void pt_plotEffStat(TFile* f,TFile* of=0 , const char* var="Z0_Pt_masscut") {
  setTDRStyle();
  ...
  TH1* fe=0,*s=0,*ss=0;
  //  TFile *mfile = new TFile(outfile.c_str(),"recreate");
  for (int i=1; i<=ntrials; i++) {
    ...
    if (i==1) {
      fe=new TH1F("MYH","FractionalError",pt_bins,pt_binning);//h_ecec->GetNbinsX(),h_ecec->GetXaxis()->GetXmin(),h_ecec->GetXaxis()->GetXmax());
      fe->GetXaxis()->SetTitle("q_{T}  [GeV]");
       if (of!=0)       fe->SetDirectory(of);
      s=new TH1F("Sum","Sum",pt_bins,pt_binning);//h_ecec->GetNbinsX(),h_ecec->GetXaxis()->GetXmin(),h_ecec->GetXaxis()->GetXmax());      
      ss=new TH1F("SumSq","SumSq",pt_bins,pt_binning);//h_ecec->GetNbinsX(),h_ecec->GetXaxis()->GetXmin(),h_ecec->GetXaxis()->GetXmax());
    }
...
void plotEffSyst(TFile* f, TFile *fhf,  TFile* of=0, const char* var="Z0_Pt_masscut", const char* varpm="Plus") {
  ...
  fe = zpt_rebinForPlot((TH1*) feb->Clone("MYH"));
  //fe->SetTitle("FractionalError");
  fec=zpt_rebinForPlot(fec);
  fe->Add(fec,-1.);
  fe->Divide(fec);
    zrap_Prelim(.8,.8);
  fe->Draw();
