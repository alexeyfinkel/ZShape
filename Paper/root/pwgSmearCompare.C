...
void pwgSmearCompare(TFile* f1,TFile* f2,TFile* f3,TFile* f4,TFile* f5,TFile* f6,TFile* f7,TFile* f8,TFile* f9,TFile* f10,TFile* f11,TFile* f12){
...
double inter =   h[k]->Integral();
  ...
  c1->Print("~/public_html/talk/appendiz_july25.png");

  c2->cd();

  double x[20]={ 0,0.25,0.5,1.0,1.5,2.25,2.5,2.75,3.0,3.25,3.5,4.0};
    double chi2[20];  
    for(int j=1;j<hn;j++){ 
      chi2[j]=0 ;
  ...
  for(int j=0;j<hn;j++){
    ...
    gr->Draw("P");
    c2->Print("~/public_html/talk/smear_chi2_parp91pt15with93matched_jun10.png");
