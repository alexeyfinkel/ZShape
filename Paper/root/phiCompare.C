...
void phiCompare(TFile* sigf, TFile* zfd_bkgd, TFile* zfd_final) {
  setTDRStyle();
  const double weight=1.0/(1350);
  char name[200];
  sprintf(name,"ZFromData/ECAL80-ECAL95/C08-m(60,120)/mZ_vs_e2_phi");
  TH2* bkshape=zfd_bkgd->Get(name)->Clone("bkgd");
  TH2* data=zfd_final->Get(name)->Clone("data");
  sprintf(name,"mcEff/ECAL80-ECAL95/C08-m(60,120)/mZ_vs_e2_phi");
  TH2* sig=sigf->Get(name)->Clone("sig");
  sprintf(name,"ZFromData/ECAL80-ECAL95/C08-m(60,120)/mZ_vs_e1_phi");
  TH2* bkshape1=zfd_bkgd->Get(name)->Clone("bkgd");
  TH2* data1=zfd_final->Get(name)->Clone("data");
  sprintf(name,"mcEff/ECAL80-ECAL95/C08-m(60,120)/mZ_vs_e1_phi");
  ...
  s2->GetXaxis()->SetTitle("Electron #phi");
  s2->GetYaxis()->SetTitle("Candidates/0.13 radians");
  ...
  c2->Print("~/public_html/talk/Z_CompareFINAL_ECAL80-ECAL95_e_phi_bkgd.eps");
}
