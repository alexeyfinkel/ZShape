...
void bkgCompare(const char* item, TFile* sigf, TFile* zfd_bkgd, TFile* zfd_final) {
  double bckinteg=0;
  setTDRStyle();
  const double weight=1.0/(1350);
  char name[200];
  sprintf(name,"ZFromData/ECAL80-HF/C08-m(60,120)/mZ_vs_%s",item);
  TH2* bkshape=zfd_bkgd->Get(name)->Clone("bkgd");
  TH2* data=zfd_final->Get(name)->Clone("data");
  sprintf(name,"mcEff/ECAL80-HF/C08-m(60,120)/mZ_vs_%s",item);
  ...
  for (int i=1; i<=data->GetNbinsX(); i++) {
    ...
    slbk->Scale(sld->Integral(1,17)*1.0/(slbk->Integral(1,17)));//for hf
  ...
  std::cout<<"@@@@@background integral  "<<bkg1d->Integral()<<std::endl; 
  bkg1d->Scale(235*1.0/bkg1d->Integral());
...
s2->GetXaxis()->SetTitle("Electron P_{T},e2 [GeV]");
//s2->GetXaxis()->SetTitle("Electron #eta,e1");
 s2->GetYaxis()->SetTitle("Candidates/2.00 GeV");
...
zrap_Prelim(0.80,0.977);
  ...
  char stuff[1024];
  sprintf(stuff,"~/public_html/talk/HF_mc_v_bkg_%s.eps",item);
  c2->Print(stuff);
