...
void pwgCompare(TFile* f1,TFile* f2,TFile* f3,TFile* f4,TFile* f5,TFile* f6,TFile* f7,TFile* f8){//,TFile* f9,TFile* f10){
...
double inter =   h[k]->Integral();
  ...
  tl->AddEntry(h[3],"Parp[91]=0.75","l");
  tl->AddEntry(h[4],"Parp[91]=1.0","l");
  tl->AddEntry(h[5],"Parp[91]=1.25","l");
  tl->AddEntry(h[6],"Parp[91]=1.5","l");
  tl->AddEntry(h[7],"Parp[91]=2.0","l");
  // tl->AddEntry(h[8],"Parp[91]=3.0","l");
//   tl->AddEntry(h[9],"Parp[91]=3.5","l");
  tl->Draw();

  c1->Print("~/public_html/talk/pwgCompare_pt5_may31.png");

  c2->cd();

  double x[20]={ 0,0.25 , 0.5 , 0.75,1.0,1.25, 1.5, 2.0};
    double chi2[20];  
    for(int j=1;j<hn;j++){
      chi2[j]=0 ;
    ...
    TH1* gr=new TH1F("name","name",13,-0.125,3.125);
  for(int j=0;j<hn;j++){
    gr->SetBinContent(gr->FindBin(x[j]),chi2[j]);
  }
    gr->Draw("P");
    c2->Print("~/public_html/talk/chi2_parp91pt5_may31.png");
