...
void app_bob2(TFile* f1,TFile* f2,TFile* f3,TFile* f4,TFile* f5,TFile* f6,TFile* f7,TFile* f8,TFile* f9){
...
zrap_Prelim(.8,.82);
  ...
  c1->Print("~/public_html/talk/appendiz_bob2_july25.eps");
  
  c2->cd();
  
  double x[20]={ 0,0.25,0.5,0.75,1.0,1.5,2.0,2.5,3.0};
  double chi2[20];  
  for(int j=0;j<hn;j++){ 
    chi2[j]=0 ;
...
zrap_Prelim(.8,.82);
  ...
  c2->Print("~/public_html/talk/appendiz_bob2Chi2_july25.eps");
